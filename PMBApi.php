			...
			if ( $row = $countpdo->fetch(PDO::FETCH_ASSOC) )
			{
				if ( $this->LoadSettings((int)$row["ID"]) )
				{
					...
					if ( !isset($row["min_id"]) || !isset($row["max_id"]) || !isset($row["max_delta_id"]) )
					{
						...
						{
							$this->min_doc_id = (int)$valrow["min_id"];
							$this->max_doc_id = (int)$valrow["max_id"];
						}
					}
					else
					{
						$this->min_doc_id = (int)$row["min_id"];
						
						if ( !empty($row["max_delta_id"]) && $row["max_delta_id"] > $row["max_id"] )
						{
							$this->max_doc_id = (int)$row["max_delta_id"];
						}
						else
						{
							$this->max_doc_id = (int)$row["max_id"];
						}
					}

					if ( !empty($row["delta_documents"]) )
					{
						$this->documents_in_collection += (int)$row["delta_documents"];
						$this->delta_documents			= (int)$row["delta_documents"];
					}
				}
				else
				{
		...
		try
		{
				...
				while ( $row = $ppdo->fetch(PDO::FETCH_ASSOC) )
				{
					...
					for ( $i = 0 ; $i < $len ; ++$i )
					{
						...
						if ( $bits > 127 )
						{
							...
							# if prefix minimum quality is set 
			...
			*/ 
			...
			else
			{
				...
				if ( !empty($non_wanted_keywords) || !empty($this->disabled_documents) ) /* || !empty($non_wanted_group) */ 
				{
