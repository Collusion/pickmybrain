            ...
            while ( true ) 
			{
				...
				foreach ( $sorted_groups as $group => $token_group ) 
				{
					...
					while ( $i ) 
					{
						...
						{
							// reads as: temp = temp << 7 | next_bits
							$temp = $this->pow_lookup[1] * $temp + $next_bits;	# the new bits get added as LSBs	
						}
						else
						{
							$delta = (string)($delta - $temp + 1); 
							$temp = $next_bits&127;
						...
						if ( ($bits & $reference_bits) === $goal_bits )
						{
							...
							for ( $i = 1 ; $i < $data_len ; ++$i )
							{
								...
								else
								{
									...
									{
										// reads as delta += bits&127 << shift*7 + $temp
